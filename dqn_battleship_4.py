...
class Agent:
    ...
    def __init__(self, gamma, epsilon, learning_rate, input_dims, batch_size, n_actions,
        ...
        self.Q_eval = DeepQNetwork(learning_rate, n_actions=n_actions,
    ...
    def chooseAction(self, observation):
        ...
        """
        choices_remaining = observation == 0
        if np.random.random() > self.epsilon:
            ...
            action = np.argmax(np.ma.array(actions.cpu().data.numpy(), mask=action_mask))
            # action = torch.argmax(actions).item()
        else:
            action = np.random.choice(np.arange(len(observation)), p=choices_remaining/choices_remaining.sum())
